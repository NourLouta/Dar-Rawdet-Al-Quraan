...
# ================================
class DarTheme:
    """ألوان هوية دار روضة القرآن — أخضر زمردي + ذهبي"""

    # PRIMARY — Deep Teal (from logo arch)
    PRIMARY        = "#1B6B5A"
    PRIMARY_DARK   = "#134E41"
    PRIMARY_LIGHT  = "#2A8C75"
    PRIMARY_GLOW   = "rgba(27,107,90,0.25)"

    # GOLD — from logo pen/book
    GOLD           = "#C9A84C"
    GOLD_DARK      = "#A8892E"
    GOLD_LIGHT     = "#E2C06A"
...
def check_password():
    ...
    * {{ font-family: {T.FONT_FAMILY}; direction: rtl; }}
    ...
    logo_html = (f'<img src="data:image/png;base64,{LOGO_B64}" style="max-width:160px;height:auto;'
                 f'filter:drop-shadow(0 4px 12px rgba(27,107,90,0.5));">'
    ...
    st.markdown("""
    <div class="login-footer">🔒 دخول آمن &nbsp;|&nbsp; دار روضة القرآن</div>
    """, unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    return False
...
def inject_global_css():
    ...
    * {{ font-family: {T.FONT_FAMILY}; }}
    ...
    section[data-testid="stSidebar"] {{
        background: {T.gradient("#0A1A16","#0F2E24",180)} !important;
        direction: rtl;
    }}
    section[data-testid="stSidebar"] * {{ color: {T.TEXT_LIGHT} !important; }}
    section[data-testid="stSidebar"] .stSelectbox label,
    section[data-testid="stSidebar"] .stMultiSelect label {{
        color: {T.GOLD_LIGHT} !important;
        font-weight: 600 !important;
    }}

    .stTabs [data-baseweb="tab-list"] {{
        gap: 8px;
        background: {T.teal_gradient(90)};
        padding: 12px 16px;
        border-radius: {T.BORDER_RADIUS_LARGE};
        box-shadow: {T.SHADOW_MEDIUM};
        direction: rtl;
    ...
    /* KPI Cards */
    .kpi-card {{
        background: {T.teal_gradient(135)};
        padding: 1.6rem 1.2rem;
        border-radius: {T.BORDER_RADIUS_LARGE};
        box-shadow: 0 10px 30px rgba(27,107,90,0.35);
        text-align: center;
    ...
    .dar-table th {{
        padding: 10px 12px; border: 1px solid {T.PRIMARY_DARK};
...
def plotly_layout(fig, title="", height=420):
    fig.update_layout(
        title=dict(
            text=f'<b style="color:{T.PRIMARY_DARK};font-size:16px;">{title}</b>',
    ...
    fig.update_xaxes(showgrid=True, gridcolor="rgba(27,107,90,0.10)",
                     linecolor=T.PRIMARY_LIGHT, tickfont=dict(size=10))
...
# ============================================================
# TAB 1 — لوحة المتابعة (OVERVIEW DASHBOARD)
# ============================================================
with tab_overview:
    ...
    if not students_df.empty:
        ...
        online_students = students_df["نظام الاشتراك"].str.contains("أونلاين|اونلاين", na=False).sum() if "نظام الاشتراك" in students_df.columns else 0
        home_students   = students_df["نظام الاشتراك"].str.contains("منزلي", na=False).sum() if "نظام الاشتراك" in students_df.columns else 0
        total_salary    = compute_total_salaries(students_df, teachers_df)
        net_profit      = total_revenue - total_salary
    else:
        active_students = frozen_students = online_students = home_students = 0
    ...
    with c6: st.markdown(kpi_card("📈", fmt_currency(net_profit), "صافي الربح", "emerald" if net_profit >= 0 else "crimson"), unsafe_allow_html=True)
    ...
    with col_b:
        if not students_df.empty and "الفئة" in students_df.columns:
            ...
            fig_cat = go.Figure(go.Bar(
                ...
                orientation="h",
                marker=dict(
                    color=CHART_COLORS[:len(cat_counts)],
    ...
    with col_d:
        if not students_df.empty and "اسم المحفظ/ة" in students_df.columns:
            teacher_load = students_df[students_df.get("حالة الاشتراك", pd.Series(dtype=str)).str.contains("نشط", na=False)]["اسم المحفظ/ة"].value_counts()
            fig_load = go.Figure(go.Bar(
                x=teacher_load.index.tolist(),
                y=teacher_load.values.tolist(),
                marker=dict(
                    color=CHART_COLORS[:len(teacher_load)],
                    line=dict(color="white", width=1)
                ),
    ...
    # ── Subscription Revenue by Teacher ─────────────────────────────────────
    if not students_df.empty:
...
with tab_students:
    ...
    if students_df.empty:
        ...
        <div class="insight-box insight-teal" style="margin-bottom:1rem;">
            <b>📊 نتائج التصفية:</b> {len(filtered)} طالب
            &nbsp;|&nbsp; إجمالي الاشتراكات:
        ...
        display_table(
            ...
            title=f"قائمة الطلاب ({len(filtered)} سجل)",
            download_name="الطلاب.csv",
            max_height="520px"
        )
...
with tab_teachers:
    ...
    if teachers_df.empty:
        ...
        display_cols_t = [c for c in [
            "ID", "الاسم", "النوع", "المحافظة",
            "المؤهل/الإجازة", "سنوات الخبرة",
            "الفئة التي يدرّسها", "نظام العمل",
            "سعر الساعة", "التوقيت", "حالة التعاقد", "مميز في", "ملاحظات"
        ] if c in teachers_df.columns]

        display_table(
            teachers_df[display_cols_t],
            title="قائمة المحفظين",
            download_name="المحفظون.csv"
        )
        ...
        if sel_t != "— اختر —":
            ...
            if not t_row.empty:
                ...
                revenue_t = pd.to_numeric(
                    t_students[t_students.get("حالة الاشتراك", pd.Series(dtype=str)).str.contains("نشط", na=False)].get("قيمة الاشتراك الشهري", pd.Series(dtype=float)),
                    errors="coerce"
                ...
                with td2:
                    ...
                    <div class="salary-card">
                        ...
                        <div style="direction:rtl;line-height:2.2;font-size:0.9rem;">
                            ...
                            💵 <b>سعر الساعة:</b> {HOURLY_RATE} ج.م<br>
                            <hr style="border-color:rgba(27,107,90,0.2);">
                            💰 <b style="font-size:1.1rem;color:{T.PRIMARY_DARK};">
                               الراتب المستحق: {fmt_currency(salary)}
                            </b><br>
        ...
        with ch1:
            if "المحافظة" in teachers_df.columns:
                ...
                fig_gov = go.Figure(go.Bar(
                    x=gov_counts.index.tolist(),
                    y=gov_counts.values.tolist(),
                    marker=dict(color=CHART_COLORS[:len(gov_counts)]),
                    text=gov_counts.values.tolist(),
                    textposition="outside",
                ))
        ...
        with ch2:
            if "المؤهل/الإجازة" in teachers_df.columns:
                ...
                fig_lic = go.Figure(go.Pie(
                    labels=lic_counts.index.tolist(),
                    values=lic_counts.values.tolist(),
                    hole=0.5,
                    marker=dict(colors=CHART_COLORS[:len(lic_counts)]),
                    textinfo="label+percent",
                    textfont=dict(size=10),
                ))
...
with tab_schedule:
    ...
    with sched_tab1:
        ...
        <div class="insight-box insight-teal">
            <b>📌 كيفية الاستخدام:</b> أدخل كود الطالب (مثال: S-00001) أو جزء من اسمه لعرض جدول حصصه الشهري.
        </div>
        """, unsafe_allow_html=True)
        ...
        if student_input.strip():
            ...
            if sched.empty:
                ...
                <div class="insight-box insight-success">
                    ✅ <b>{student_name}</b> — المحفظ/ة: <b>{teacher_name}</b>
                    &nbsp;|&nbsp; عدد الحصص: <b>{len(sched)}</b>
                </div>
                """, unsafe_allow_html=True)
    ...
    with sched_tab2:
        ...
        <div class="insight-box insight-gold">
            <b>📌 كيفية الاستخدام:</b> اختر اسم المحفظ/ة لعرض جدول جميع حصصه/ها مع طلابه/ها هذا الشهر.
        </div>
        """, unsafe_allow_html=True)

        teacher_names_sched = ["— اختر —"]
        if not teachers_df.empty and "الاسم" in teachers_df.columns:
            teacher_names_sched += teachers_df["الاسم"].dropna().tolist()
        ...
        if sel_t_sched != "— اختر —":
            ...
            if t_sched.empty:
                ...
                display_table(
                    t_sched,
                    title=f"جدول حصص {sel_t_sched}",
                    download_name=f"جدول_{sel_t_sched}.csv"
                )

                # Sessions per student bar chart
                if "اسم الطالب" in t_sched.columns:
                    sess_per_student = t_sched["اسم الطالب"].value_counts()
                    fig_tss = go.Figure(go.Bar(
                        x=sess_per_student.index.tolist(),
                        y=sess_per_student.values.tolist(),
                        marker=dict(color=T.PRIMARY, line=dict(color=T.PRIMARY_DARK, width=1)),
                        text=sess_per_student.values.tolist(),
                        textposition="outside",
                    ))
    ...
    with sched_tab3:
        ...
        if source_month.empty:
            ...
            if all_month_sessions:
                ...
                st.markdown(f"""
                <div class="insight-box insight-teal">
                    📊 <b>إجمالي الحصص المعروضة:</b> {len(month_sched_df)} حصة
                </div>
                """, unsafe_allow_html=True)

                display_table(
                    month_sched_df,
                    title="جدول الشهر الكامل",
                    download_name="جدول_الشهر_الكامل.csv",
                    max_height="560px"
                )
...
with tab_search:
    ...
    with auto_tab1:
        st.markdown(f"""
        <div class="insight-box insight-teal">
            <b>🔍 البحث الموحد:</b> ابحث بكود الطالب أو المحفظ أو الاسم أو رقم الهاتف في جميع البيانات.
        </div>
        """, unsafe_allow_html=True)
        ...
        if search_query.strip():
            ...
            if not students_df.empty:
                for col in students_df.columns:
                    try:
                        mask = students_df[col].astype(str).str.contains(q, case=False, na=False)
                        if mask.any():
                            matched = students_df[mask].copy()
                            matched["مصدر البحث"] = f"الطلاب — عمود: {col}"
            ...
            if not teachers_df.empty:
                for col in teachers_df.columns:
                    try:
                        mask = teachers_df[col].astype(str).str.contains(q, case=False, na=False)
                        if mask.any():
                            matched = teachers_df[mask].copy()
                            matched["مصدر البحث"] = f"المحفظون — عمود: {col}"
                            results_list.append(matched)
                    except Exception:
                        pass
            ...
            if results_list:
                ...
                display_table(combined_results, title="نتائج البحث", download_name=f"بحث_{q}.csv", max_height="480px")
            else:
    ...
    with auto_tab2:
        st.markdown(f"""
        <div class="insight-box insight-gold">
            <b>📅 مولّد الجداول:</b> أدخل كود الطالب أو المحفظ لتوليد جدول الأسبوع أو الشهر القادم تلقائياً.
        </div>
        """, unsafe_allow_html=True)
        ...
        with gen_col2:
            ...
            if not teachers_df.empty and "الاسم" in teachers_df.columns:
                teacher_names_gen += teachers_df["الاسم"].dropna().tolist()
    ...
    with auto_tab3:
        st.markdown(f"""
        <div class="insight-box insight-teal">
            <b>💰 حاسبة الرواتب:</b> احسب راتب أي محفظ/ة بناءً على ساعات العمل الفعلية المسجلة.
        </div>
        """, unsafe_allow_html=True)
        ...
        with sc_col1:
            ...
            if not teachers_df.empty and "الاسم" in teachers_df.columns:
                teacher_names_sal += teachers_df["الاسم"].dropna().tolist()
                ...
                if sel_sal_teacher != "— اختر —":
                    ...
                    <div class="salary-card">
                        ...
                        <div style="direction:rtl;line-height:2.5;font-size:1rem;text-align:center;">
                            ...
                            <span style="font-size:1.5rem;font-weight:900;color:{T.PRIMARY_DARK};">
                                الراتب المستحق: {fmt_currency(salary_c)}
                            </span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
        ...
        with sc_col2:
            ...
            if not teachers_df.empty and "الاسم" in teachers_df.columns:
                compare_rate = st.number_input("سعر الساعة للمقارنة (ج.م)", min_value=1, value=HOURLY_RATE, step=5, key="compare_rate")
                if st.button("📊 عرض مقارنة الرواتب", key="btn_compare_sal"):
                    ...
                    for _, t in teachers_df.iterrows():
                        ...
                        compare_records.append({"المحفظ/ة": t_name, "الساعات": h, "الراتب": s})
                    ...
                    if compare_records:
                        ...
                        fig_comp = go.Figure(go.Bar(
                            ...
                            marker=dict(color=CHART_COLORS[:len(comp_df)]),
                        ...
                        display_table(comp_df, download_name="مقارنة_الرواتب.csv")
        ...
        if st.button("🚀 توليد التقرير الشهري الشامل", key="btn_full_report"):
            with st.spinner("⏳ جاري إعداد التقرير..."):
                ...
                if not students_df.empty:
                    ...
                    with rk4: st.markdown(kpi_card("💰", fmt_currency(total_rev), "الإيرادات", "gold"), unsafe_allow_html=True)
                ...
                if not teachers_df.empty and "الاسم" in teachers_df.columns:
                    ...
                    report_sal_df = pd.DataFrame(report_sal_records)
                    display_table(report_sal_df, download_name="تقرير_الرواتب_الشهري.csv")
                ...
                if all_r_sessions:
                    full_sched_df = pd.DataFrame(all_r_sessions)
                    display_table(full_sched_df, download_name="الجدول_الشهري_الكامل.csv", max_height="400px")
                ...
                <div class="salary-card" style="text-align:center;">
                    ...
                    <div style="direction:rtl;line-height:3;font-size:1rem;">
                        ...
                        <span style="color:{T.GOLD};font-size:1.2rem;font-weight:800;">{fmt_currency(total_rev)}</span><br>
                        💸 <b>إجمالي الرواتب:</b>
                        <span style="color:{T.ACCENT_CRIMSON};font-size:1.2rem;font-weight:800;">{fmt_currency(total_sal)}</span><br>
                        <hr style="border-color:rgba(27,107,90,0.3);margin:0.5rem auto;width:60%;">
                        📈 <b>صافي الربح:</b>
                        <span style="color:{T.PRIMARY_DARK};font-size:1.5rem;font-weight:900;">{fmt_currency(net_prof)}</span><br>
                        📊 <b>هامش الربح:</b>
                        <span style="color:{T.ACCENT_SAPPHIRE};font-size:1.2rem;font-weight:800;">{profit_margin:.1f}%</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
