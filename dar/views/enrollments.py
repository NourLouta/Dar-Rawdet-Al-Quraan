...
**ما هي؟** «التسجيل» هو الجسر بين الطالب والمحفظة: يحدّد البرنامج، السعر (للطالب
وللمحفظة)، وأيام/أوقات الحصص الأسبوعية. **هذه الشاشة ضرورية قبل توليد أي حصص.**

**متى تستخدمها؟**
- بعد تسجيل طالب جديد، لربطه بمحفظة وتحديد جدوله — الخطوة التالية مباشرة.
- عند تغيّر جدول طالب أو سعره (كثّف الحصص، غيّر معاده، تغيّر السعر).
- عند إيقاف أو إنهاء تعاون طالب مع محفظة معيّنة.

**الخطوة التالية:** بعد حفظ التسجيل هنا، اذهبي إلى «📅 الحصص والإدخال ← ⚡ توليد
حصص تسجيل» لتوليد حصص الشهر تلقائيًا من هذا الجدول.
""")
    data = state.get_data()
    ...
    with t_add:
        ui.guide("متى تستخدم هذا التبويب؟", """
عند بدء تعاون جديد بين طالب ومحفظة. اختاري البرنامج فتظهر أسعاره الافتراضية تلقائيًا
(يمكن تعديلها يدويًا لو كان السعر مختلفًا لهذا الطالب تحديدًا — مثل درس منزلي بسعر أعلى).
حدّدي أيام الأسبوع ووقت/مدة كل يوم (يمكن أن تختلف من يوم لآخر).

**حلقة مشتركة بين أكثر من طالب؟** سجّلي كل طالب في تسجيل مستقل بنفس المحفظة والجدول،
واكتبي في تعديل الأسعار ما يخص كل واحد منهم على حدة.
""")
    ...
    with t_edit:
        ui.guide("متى تستخدم هذا التبويب؟", """
لتعديل سعر أو جدول تسجيل قائم (مثل تغيير المعاد، أو تعديل السعر بعد اتفاق جديد)، أو
لإيقافه/حذفه. **تنبيه:** تعديل الجدول هنا لا يُطبَّق تلقائيًا على الحصص التي وُلِّدت
سابقًا — أعيدي توليد حصص الشهر من «الحصص والإدخال» بعد أي تعديل في الجدول.
""")
        can = state.write_banner()
        if enroll.empty:
